...
import "./interface/IReferralHub.sol";
...
contract ReferralHub is IReferralHub, Ownable, ReentrancyGuard {
    ...
    function getNodehash(string calldata name, string calldata tld)
        public
        pure
        returns (bytes32)
    {
        bytes32 nameHash = keccak256(bytes(name));
        bytes32 tldHash = keccak256(
            abi.encodePacked(bytes32(0), keccak256(bytes(tld)))
        );
    ...
    function isReferralEligible(bytes32 nodeHash)
        external
        view
        override
    ...
    function getReferralCommisionFee(uint256 price, bytes32 nodeHash)
        public
        view
    ...
    function addNewReferralRecord(bytes32 referrerNodeHash)
        external
        override
        onlyController
    {
    ...
    function _getReferralCount(bytes32 referrerNodeHash)
        internal
        view
        returns (uint256)
    {
