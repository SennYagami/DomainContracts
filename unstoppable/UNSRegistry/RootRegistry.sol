...
import "./interface/IRootRegistry.sol";
...
abstract contract RootRegistry is ERC721Upgradeable, IRootRegistry {
    ...
    bytes32 internal constant _ROOT_CHAIN_MANAGER_SLOT =
        0xbe2bb46ac0377341a1ec5c3116d70fd5029d704bd46292e58f6265dd177ebafe;

    modifier onlyPredicate() {
        require(
            _msgSender() == _getPredicate(),
    ...
    function __RootRegistry_init(address rootChainManager)
        internal
        onlyInitializing
    {
    ...
    // solhint-disable-next-line func-name-mixedcase
    function __RootRegistry_init_unchained(address rootChainManager)
        internal
        onlyInitializing
    {
        StorageSlotUpgradeable
            .getAddressSlot(_ROOT_CHAIN_MANAGER_SLOT)
            .value = rootChainManager;
    }
    ...
    function _deposit(address to, uint256 tokenId) internal {
        ...
        address manager = StorageSlotUpgradeable
            .getAddressSlot(_ROOT_CHAIN_MANAGER_SLOT)
            .value;
        IRootChainManager(manager).depositFor(
            to,
            address(this),
            abi.encode(tokenId)
        );
    ...
    function _getPredicate() internal view returns (address predicate) {
        address manager = StorageSlotUpgradeable
            .getAddressSlot(_ROOT_CHAIN_MANAGER_SLOT)
            .value;
        bytes32 tokenType = RootChainManagerStorage(manager).tokenToType(
            address(this)
        );
